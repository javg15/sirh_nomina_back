...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_cattiposadeudos_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_cattiposadeudos_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Cattiposadeudos.findOne({
        ...
        .then(cattiposadeudos => {
            if (!cattiposadeudos) {
                return res.status(404).send({ message: "Cattiposadeudos Not found." });
            }

            res.status(200).send(cattiposadeudos);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
    ...
    Cattiposadeudos.findOne({
        ...
        .then(cattiposadeudos => {
            if (!cattiposadeudos) {
                ...
                cattiposadeudos.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
