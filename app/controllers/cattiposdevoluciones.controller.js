...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_cattiposdevoluciones_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_cattiposdevoluciones_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Cattiposdevoluciones.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(cattiposdevoluciones => {
            if (!cattiposdevoluciones) {
                return res.status(404).send({ message: "Cattiposdevoluciones Not found." });
            }

            res.status(200).send(cattiposdevoluciones);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getCatalogo = async(req, res) => {
    ...
    Cattiposdevoluciones.findAll({
            attributes: ['id', 'descripcion','condias'],
            order: [
                ['descripcion', 'ASC'],
            ]
        }).then(cattiposdevoluciones => {
    ...
    Cattiposdevoluciones.findOne({
        ...
        .then(cattiposdevoluciones => {
            if (!cattiposdevoluciones) {
                ...
                cattiposdevoluciones.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
