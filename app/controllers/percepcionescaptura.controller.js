...
const Percepcionescaptura = db.percepcionescaptura;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM nomina.s_percepcionescaptura_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM nomina.s_percepcionescaptura_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Percepcionescaptura.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(percepcionescaptura => {
            if (!percepcionescaptura) {
                return res.status(404).send({ message: "Percepcionescaptura Not found." });
            }

            res.status(200).send(percepcionescaptura);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.setRecord = async(req, res) => {
    Object.keys(req.body.dataPack).forEach(function(key) {
        ...
        if (key.indexOf("importe", 0) >= 0
            || key.indexOf("diasapagar", 0) >= 0
            || key.indexOf("cantidadquincenas", 0) >= 0) {
            if (req.body.dataPack[key] != '')
                req.body.dataPack[key] = parseInt(req.body.dataPack[key]);
        }
    })

    let percepcionesCapturaExiste=false;
    //revisar si la combinación de campos, ya existe
    await Percepcionescaptura.findOne({
        where: {
            [Op.and]: [
    ...
    .then(percepcionescaptura => {
        if (percepcionescaptura) {
            percepcionesCapturaExiste = true;
        }
    });
    ...
    const dataVSchema = {
        ...
        id_personal: { type: "number" ,
            custom(value, errors) {
                if(percepcionesCapturaExiste) errors.push({ type: "uniqueRecord" })
                if(value <= 0) errors.push({ type: "selection" })
        ...
        importetotal: { type: "number" ,
            custom(value, errors) {
                if(value <= 0 && req.body.dataPack["diasapagar"]<=0) errors.push({ type: "required" })
    ...
    Percepcionescaptura.findOne({
        ...
        .then(percepcionescaptura => {
            if (!percepcionescaptura) {
                ...
                percepcionescaptura.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
