...
const Deduccionescaptura = db.deduccionescaptura;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM nomina.s_deduccionescaptura_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM nomina.s_deduccionescaptura_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Deduccionescaptura.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(deduccionescaptura => {
            if (!deduccionescaptura) {
                return res.status(404).send({ message: "Deduccionescaptura Not found." });
            }

            res.status(200).send(deduccionescaptura);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.setRecord = async(req, res) => {
    ...
    let deduccionesCapturaExiste=false;
    //revisar si la combinación de campos, ya existe
    await Deduccionescaptura.findOne({
        where: {
            [Op.and]: [
                { id_personal: req.body.dataPack.id_personal },
                { id_catdeducciones: req.body.dataPack.id_catdeducciones },
    ...
    const dataVSchema = {
        ...
        id_personal: { type: "number" ,
            custom(value, errors) {
                if(deduccionesCapturaExiste) errors.push({ type: "uniqueRecord" })
                if(value <= 0) errors.push({ type: "selection" })
    ...
    Deduccionescaptura.findOne({
        ...
        .then(deduccionescaptura => {
            if (!deduccionescaptura) {
                ...
                deduccionescaptura.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
