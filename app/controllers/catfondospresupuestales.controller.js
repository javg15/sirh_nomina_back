...
const Catfondospresupuestales = db.catfondospresupuestales;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catfondospresupuestales_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catfondospresupuestales_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catfondospresupuestales.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catfondospresupuestales => {
            if (!catfondospresupuestales) {
                return res.status(404).send({ message: "Catfondospresupuestales Not found." });
            }

            res.status(200).send(catfondospresupuestales);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getCatalogo = async(req, res) => {
    ...
    Catfondospresupuestales.findAll({
    ...
    Catfondospresupuestales.findOne({
        ...
        .then(catfondospresupuestales => {
            if (!catfondospresupuestales) {
                ...
                Catfondospresupuestales.create(
                    req.body.dataPack
                ).then((self) => {
                ...
                catfondospresupuestales.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
