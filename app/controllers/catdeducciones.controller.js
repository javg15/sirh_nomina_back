...
const Catdeducciones = db.catdeducciones;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM nomina.s_catdeducciones_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM nomina.s_catdeducciones_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catdeducciones.findOne({
        ...
        .then(catdeducciones => {
            if (!catdeducciones) {
                return res.status(404).send({ message: "Catdeducciones Not found." });
            }

            res.status(200).send(catdeducciones);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
    ...
    Catdeducciones.findOne({
        ...
        .then(catdeducciones => {
            if (!catdeducciones) {
                ...
                catdeducciones.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
