...
const Catrecibosestatus = db.catrecibosestatus;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catrecibosestatus_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catrecibosestatus_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catrecibosestatus.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catrecibosestatus => {
            if (!catrecibosestatus) {
                return res.status(404).send({ message: "Catrecibosestatus Not found." });
            }

            res.status(200).send(catrecibosestatus);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
    ...
    Catrecibosestatus.findOne({
        ...
        .then(catrecibosestatus => {
            if (!catrecibosestatus) {
                ...
                catrecibosestatus.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
