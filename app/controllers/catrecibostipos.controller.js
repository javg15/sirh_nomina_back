...
const Catrecibostipos = db.catrecibostipos;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catrecibostipos_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catrecibostipos_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catrecibostipos.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catrecibostipos => {
            if (!catrecibostipos) {
                return res.status(404).send({ message: "Catrecibostipos Not found." });
            }

            res.status(200).send(catrecibostipos);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
    ...
    Catrecibostipos.findOne({
        ...
        .then(catrecibostipos => {
            if (!catrecibostipos) {
                ...
                catrecibostipos.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
