...
const Compensaciones = db.compensaciones;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM nomina.s_compensaciones_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM nomina.s_compensaciones_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Compensaciones.findOne({
        ...
        .then(compensaciones => {
            if (!compensaciones) {
                return res.status(404).send({ message: "Compensaciones Not found." });
            }

            res.status(200).send(compensaciones);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.setRecord = async(req, res) => {
    ...
    ultimoNumero=await Compensaciones.findOne({
        attributes: ['numero'],
        where: {
            [Op.and]: [
                { state: "A" }],
    ...
    .then(compensaciones => {
        if (!compensaciones) {
            ...
            compensaciones.update(req.body.dataPack).then((self) => {
                // here self is your instance, but updated
                res.status(200).send({ message: "success", id: self.id });
            });
        }
